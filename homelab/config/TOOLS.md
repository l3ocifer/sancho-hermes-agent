...
What's wired up, where it lives, and how to use it. Lives in `~/.hermes/
workspace/TOOLS.md` after deploy.
...
- **Pod**: `sancho/sancho` Deployment, nodeSelector `kubernetes.io/
  hostname: alef`, single replica
...
3. The `sancho-persona` ConfigMap in
   [`argocd/apps/_agents/sancho/configmap-persona.yaml`](../../
   argocd/apps/_agents/sancho/configmap-persona.yaml) auto-rolls
   the deployment via ArgoCD (or `kubectl rollout restart deploy/
   sancho` for an immediate flip)
